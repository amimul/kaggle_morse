...
class Morse:
    ...
    def pnn(self, m, s):
        ...
        w = [[0.1, 0.1], [0.1, 0.3], [0.1, 0.7], [0.3, 0.1], [0.3, 0.3], [0.3, 0.7], [0.00, 0.05], [0.000, 0.5],
             [0.0, 0.8]]
...
def decode_stream(signal, samplerate):
    ...
    # 移动平均滤波器平滑信号包络 - 减少噪声尖峰
    # print(np.ones(bfv))
    env = np.resize(np.convolve(signal, np.ones(int(bfv)) / bfv), len(signal))
    mx = np.nanmax(env)
    mn = np.nanmin(env)
    mean = np.mean(env)
...
def process(fname):
   ...
   # sys.stdout.write(b[1])
    ...
    if fft_scan or plotter:
        ...
        s_len=int(len(s) / 2 - 1)
        f_len=int(len(f) / 2 - 1)
        maxtab, mintab = peakdet(abs(s[0:s_len]), threshold, f[0:f_len])

        # 或者画图
        plt.plot(f[0:f_len], abs(s[0:s_len]), 'g-')
        print(maxtab)
